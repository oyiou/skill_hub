...
function(__qt_internal_add_static_plugins_once)
    set(_module_target "Qt6::Network")
    set(_qt_plugins "")
    ...
    file(GLOB _qt_plugin_config_files "${CMAKE_CURRENT_LIST_DIR}/Qt6*PluginConfig.cmake")
    foreach(_config_file ${_qt_plugin_config_files})
        string(REGEX REPLACE "^.*/Qt6(.*Plugin)Config.cmake$" "\\1" _qt_plugin "${_config_file}")
        include("${_config_file}")
        if(TARGET "Qt6::${_qt_plugin}")
    ...
    foreach(plugin_target ${_qt_plugins})
        ...
        __qt_internal_add_static_plugin_import_macro(
            "${plugin_target}" ${_module_target} "Network")
    endforeach()
...
if(NOT OFF AND NOT QT_NO_CREATE_TARGETS)

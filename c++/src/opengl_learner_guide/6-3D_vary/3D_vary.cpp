...
int main()
{
    ...
    /* 窗口捕获与处理 */
    GLFWwindow* window_1 = glfwCreateWindow(WIDTH, HEIGHT, "OpenGL Earth-moon System", nullptr, nullptr);
...
//    glewExperimental = GL_TRUE;
    ...
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))			// 判断glad初始化是否成功
    {
    ...
    while (!glfwWindowShouldClose(window_1))
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            if( i == 0 )					// 大立方体
            {
                transform_1 = glm::scale(transform_1, glm::vec3(0.6f, 0.6f, 0.6f));                                     // 缩放处理
                transform_1 = glm::rotate(transform_1, time, glm::vec3(0.2f, 1.0f, 0.0f));                          // 旋转处理
            }
            else							// 小立方体
            {
                transform_1 = glm::translate(transform_1, glm::vec3(0.8f * cos(time), 0.8f * sin(time), 0.0f));     // 平移处理
                transform_1 = glm::scale(transform_1, glm::vec3(0.15f, 0.15f, 0.15f));                                  // 缩放处理
                transform_1 = glm::rotate(transform_1, time, glm::vec3(0.5f, 0.5f, 0.0f));                          // 旋转处理
            }

...
void FileView::slotShow(const QDir& dir) {
    QStringList string;
    string << "*";
...
void FileView::showFileInfoList(const QFileInfoList& list) {
    fileListWidget->clear();
    for (unsigned int i = 0; i < list.count(); i++) {
        const QFileInfo& tmpFileInfo = list.at(i);
        if (tmpFileInfo.isDir()) {
            QIcon icon("dir.png");
            QString fileName = tmpFileInfo.fileName();
            auto *tmp = new QListWidgetItem(icon, fileName);

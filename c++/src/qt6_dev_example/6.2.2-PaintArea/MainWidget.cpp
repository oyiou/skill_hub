...
MainWidget::MainWidget(QWidget *parent): QWidget(parent) {
    ...
    spreadComboBox->addItem(tr("Odd Even"), QGradient::PadSpread);
    spreadComboBox->addItem(tr("Winding"), QGradient::RepeatSpread);
    spreadComboBox->addItem(tr("Winding"), QGradient::ReflectSpread);
...
void MainWidget::showPenColor() {
    ...
    Qt::PenCapStyle capStyle = Qt::PenCapStyle(penCapComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenJoinStyle joinStyle = Qt::PenJoinStyle(penJoinComboBox->itemData(penJoinComboBox->currentIndex(), Qt::UserRole).toInt());
    paintArea->setPen(QPen(color, value, style, capStyle, joinStyle));
}
...
void MainWidget::showPenCap(int capValue) {
    QColor color = penColorFrame->palette().color(QPalette::Window);
    int value = penWidthSpinBox->value();
    Qt::PenStyle style = Qt::PenStyle(penStyleComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenCapStyle capStyle = Qt::PenCapStyle(penCapComboBox->itemData(capValue, Qt::UserRole).toInt());
    Qt::PenJoinStyle joinStyle = Qt::PenJoinStyle(penJoinComboBox->itemData(penJoinComboBox->currentIndex(), Qt::UserRole).toInt());
    paintArea->setPen(QPen(color, value, style, capStyle, joinStyle));
}
...
void MainWidget::showPenJoin(int joinValue) {
    ...
    Qt::PenStyle style = Qt::PenStyle(penStyleComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenCapStyle capStyle = Qt::PenCapStyle(penCapComboBox->itemData(penCapComboBox->currentIndex(), Qt::UserRole).toInt());
    Qt::PenJoinStyle joinStyle = Qt::PenJoinStyle(penJoinComboBox->itemData(joinValue, Qt::UserRole).toInt());
    paintArea->setPen(QPen(color, value, style, capStyle, joinStyle));
}

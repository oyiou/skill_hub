...
const char *error_403_form = "You do not have permission to get from this server.\n";
...
http_conn::LINE_STATUE http_conn::parse_line() {
    ...
    for (; m_checked_idx < m_read_idx; ++m_checked_idx) {
        ...
        if (temp == '\r') {
            ...
            if ((m_checked_idx > 1) && (m_read_idx - 1) == '\r') {
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text) {
    ...
    if (text[0] == '\0') {
        ...
        // 否则说明已经得到了一个完整的 HTTP 请求
        return GET_REQUEST;
    } else if (strncasecmp(text, "Connection:", 11) == 0) { // 处理 Connection 头部字段
        text += 11;
        text += strspn(text, " \t");
...
// 当得到一个完整、正确的 HTTP 请求时，就分析目标文件的属性。如果目标文件存在、对所有用户可读，且不是目录，则使用 mmap 将其映射到内存地址 m_file_address 处，并告诉调用者获取文件成功
...
bool http_conn::write() {
    ...
    while (1) {
        ...
        if (temp <= -1) {
            // 如果 TCP 写缓冲区没有空闲，则等待下一轮 EPOLLOUT 事件。虽然在此期间，服务器无法立即接收到同一客户的下一个请求，但这可以保证连接的完整性
            if (errno == EAGAIN) {
